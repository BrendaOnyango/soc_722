...
## PS 03: Data Wrangling

To prepare for the data wrangling exercises I loaded the Texas housing data.
```{r}
data(txhousing)
glimpse(txhousing)
```
...
#### Question 1
I viewed the data using View in the console.  
The last city listed in the data set is Wichita Falls. 

#### Question 2
I used the help function to view descriptions of the variables. 

```{r}
?txhousing
```

"Listings" describes total active listings.

#### Question 3 
I will write a chunk to remove the inventory variable and save the results in a dataframe called txhousing.

```{r}
txhousing <- txhousing |> select(-inventory)
```

There are now 8 variables instead of 9. 

#### Question 4
I will now make a data set called dallas_sub with only data from 2012 and 2013. 

```{r}
dallas_sub <- txhousing |> filter(year == 2012 | year == 2013, city == "Dallas")
```

There are 24 observations across 8 variables for Dallas in years 2012 and 2013 (remember that the inventory variable was removed in an earlier chunk).

#### Question 5

I will add a column to the dallas_sub data with a new variable for percentage of listings that were sold. 

```{r}
dallas_sub <- dallas_sub |> mutate(prct_sold = sales/listings * 100)
```

There are now 9 variables in the Dallas data set.

#### Question 6 
Now I will calculate the average percent of listings sold in Dallas grouped by month and save the results in a data frame called dallas_summary. 

```{r}
dallas_summary <- dallas_sub |> group_by(month) |> summarize(prct_sold = mean(prct_sold))
```

#### Question 7 
I will arrange the data in dallas_summary in descending order. 

```{r}
dallas_summary |> arrange(desc(prct_sold))
```

This arrangement shows that August had the greatest average of percentage of listings sold. 
#### Question 8 
I will run and explain the following code. 
```{r}
txhousing |>  
...
This code first filtered the Texas housing data by years to include only 2012 and 2013. It also filtered by city to include only data from Dallas. It created a variable called prct_sold and grouped averages of this new variable by month for 2012 and 2013. It arranged the summary data of this new variable in descending order. It is essentially one chunk that contains all the prior steps from questions 4-7. 

#### Question 9 

I will wrote a chunk to rank listings in ascending order by city in January 2015. 

```{r}
txhousing |> filter(year == 2015, month == 1) |> arrange(listings)
```
The tibble shows that San Marcos at 85 listings had the fewest in January 2015. 

#### Question 10 

I will now write a chunk to display houses sold in descending order in Texas in 2012.

```{r}
txhousing |> filter(year == 2012) |> arrange(desc(sales))
```
In 2012, the month with the most sales in Texas was August. 

#### Question 11

I will create a table that shows total number of houses sold in Austin in 2000 and 2001, and the total number of houses sold in Dallas in 2000 and 2001. 

```{r}
austin <- txhousing |> filter(city == "Austin") #creating austin set
austin |> filter (year == 2000 | year == 2001) #filtering austin set by relevant years

```
Now I will create an object that is the sum of Austin's sales for 2001-2002. 
```{r}
 austin |> mutate(austin_sum = sum(austin$sales))

austin_sum <- sum(austin$sales) #Making sure austin_sum is added as a column. 
```
Next I will repeat the steps above for Dallas. 
```{r}
dallas <- txhousing |> filter(city == "Dallas") #creating Dallas set
dallas |> filter (year == 2000 | year == 2001) 
```
Generating dallas_sum 

```{r}
dallas |> mutate(dallas_sum = sum(dallas$sales))

dallas_sum <- sum(dallas$sales) 
```
Next I will create a dataset and table with the relevant information from both Austin and Dallas. 

```{r}
rbind(austin, dallas)
```
Will name this set austin_dallas
```{r}
austin_dallas <- rbind(austin, dallas)
```
Will remove all data but for sales and year. 
```{r}
austin_dallas <- austin_dallas |> filter(year == 2000 | year == 2001) |> select(city, year, sales)
```
I will group by city and sum by group. 
```{r}
austin_dallas |> group_by(city) |> summarize(salesum = sum(sales))
```
The tibble above shows correct sums  for house sales in 2000 and 2001 for Austin and Dallas. Earlier tables showed values that weren't filtered by year. 
