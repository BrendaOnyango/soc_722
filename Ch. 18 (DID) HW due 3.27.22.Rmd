...
i. is wrong because we don't want to know if the year is 2016 or not; we want to know if we're pre or post 2015. ii is getting closer to what we need for regression, but is missing an indicator for treated where the coefficient on treated (having voter protection laws) is our DID effect. 
...
```{r}
class(sr2$date) #now the class is appearing as date 
```


```{r}

head(sr2) #checking to see format of date
tail(sr2)
```
...
```{r}
sr3 <- sr2 |> group_by(Treated = keyword == 'sourdough') |> filter(date == 2020-03-15)

#not sure how this data set ended up with 0 data. Hopefully we can go over some data cleaning issues like this on Tuesday. 
                     
```

If the prior code had worked correctly I would have ~300 or observations, not 0 and then would do this 
  priortrendstest <- feols(hits ~ Treated|date, data = sr3)
msummary(priortrendstest, stars = TRUE). Can we discuss why this didn't work in class? 

Trying another way: 

```{r}
sr4 <- sr2 |> arrange(ymd(sr2$date)) #arranging dataset by date to identify data in pretreatment period 
```

```{r}
sr5 <- sr4 |> filter(row_number() < 297) #filtering everything before 3/15/2020 (treatment date)
```

```{r}
sr5 <- sr5 |> mutate(Treated = keyword == 'sourdough') #creating treatment variable 
```


```{r}
priortrendstest <- feols(hits ~ Treated, data = sr5)
msummary(priortrendstest, stars = TRUE) #doing priortrends test 
```
This result is concerning. I'm interpreting the coefficient on TreatedTrue as meaning Treated (or sourdough) is associated with fewer hits in the pre-treatment group than non-treated. It is significant at a 99% level. If the asssumption of prior trends holds then the coefficient should be closer to 0. Based on the model and results here I must reject equal prior trends in this model. 
...
```{r}
sr7 <-  sr2 |> mutate('month' = month(date))
head(sr7) #checking to see if I have month variable 
tail(sr7)
```
Month is following standard convention of 1 = January and 12 = December. 

### 6 

Estimate a difference-in-difference model allowing the effect to differ by month (using `month = 0` as a reference period), with standard errors clustered at the keyword level, and show the results.

```{r}
#treatment variable in dataset with month variable 
sr7 <- sr7 |> mutate(Treat = keyword == 'sourdough')
```


```{r}
diffindiff <- feols(hits ~i(month, Treat, ref = 1) |  keyword + month, data = sr7)
#interacting month with being in the treated group and using January as reference month (1) since trying to use 0 resulted in an error
...
```{r}
coefplot(diffindiff, stars = TRUE) #usiing coef for graph of effects 
```


The 95% confidence interval for hits includes 0 in June - August  and in Fabruary that's displayed on this plot. My interpretation is that hits for sourdough do increase and peak in May 2020 relative to where they were in January 2020, and then decrease, but in a way that is not statistically significant i.e. while sourdough searches did increase for some months in the early pandemic the CI of the increase in several months includes 0.  
