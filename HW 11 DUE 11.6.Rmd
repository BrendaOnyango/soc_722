...
```{r}
#visual summary of coffee_beta_1
plot_normal(mean = 7.5, sd = .2) + labs(x = "beta_1", y = "pdf")
```
...
```{r}
stan_coffee_sim <- 
  stan(model_code = stan_coffee_model, 
       data = list(n = nrow(new_coffee), Y = new_coffee$total_cup_points, X = new_coffee$aroma), 
       chains = 4, iter = 5000*2, seed = 84735)
```
...
coffee_prediction <- coffee_model_df |> 
  mutate(mu = `(Intercept)` + aroma*6.163, #used slope estimate from part a 
         y_new = rnorm(20000, mean = mu, sd = sigma))
```

```{r}
ggplot(coffee_prediction, aes(x = y_new)) + 
  geom_density()
```
